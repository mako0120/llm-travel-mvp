...
- **約束→提供→次の約束(オープンループ)**: 各幕の最後の発言は、単なるまとめで
  ...
  中身に入っていこう」)
- **1行は短く、テンポよく(2026-07-23〜)**: 「画面の情報量は少なく、どんどんスムーズに
  流れていく感じにしたい」というオーナー指示を受け、各行は1〜2文以内の短い一言にする。
  長い説明を1行に詰め込まない(`docs/04_PowerPoint.md`の「1スライド1メッセージ」と対)。
  これにより1スライドの表示時間が短くなり、動画として次々に場面が切り替わるテンポが生まれる

### 追加ルール(2026-07-24〜、「伸びている系統」の分析を反映)

`research/2026-07-24_viral-retention-analysis.md` の調査に基づく。「ネットで伸びている
ものを分析し、視聴維持率を徹底的に分析して同じ系統で攻める」というオーナー指示による。

- **コールドオープン+具体的な数字を最優先にする**: 起の最初の発言(1行目)は、
  問いかけだけでなく、可能な限り具体的な数字・固有名詞をその場で提示する
  (例: 「通算10人目」「1日違いで発表」)。挨拶・前置きは省く。冒頭10秒で約20%、
  30秒で約40%が離脱するというデータがあるため、最初の一言の密度を最優先で高める
- **15秒に1回、新しい情報を出すペースを意識する**: 対話1セグメントが間延びしないよう、
  同じ事実の言い換えだけで1つのスライド・セグメントを終わらせない
- **情報密度を意図的に均一にする**: 特定のスライドに情報が偏り、前後が薄くなる
  構成を避ける(既存の「文字を減らし図解で伝える」ルールと併用)
- **冒頭60秒以内にテーマの核心キーワードを音声で発話する**: 検索・レコメンド
  アルゴリズムが音声の文字起こしをシグナルとして使うため、テーマ名・固有名詞を
  早い段階で発話する
- **効果の誇張はしない**: 視聴維持率を意識した構成上の工夫であり、「必ず伸びる」
  「絶対に見られる」といった断定的な効果を成果物内で主張しない
  (既存の正直さ・誇張回避ルールを優先する)
