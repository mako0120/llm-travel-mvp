...
## 追記(2026-07-23): 動画向けテンポに再構成(47枚)

オーナーから「もっと見たくなるように」「画面の情報量は少なく、どんどんスムーズに
流れていく感じにしたい」とのフィードバックを受け、30枚版を作り直した。カード4〜6項目・
表6行など複数要素を1枚に詰め込んでいた箇所を「1スライド1メッセージ」に分解し、
30枚→47枚に増やした一方、ナレーション各行を1〜2文以内に短縮したことで対話音声は
約9分→約3分に短縮。同一レイアウトが4枚以上連続しないよう、分解した項目群は
`cards`(明背景)と`big_stat`(暗背景)を交互に使い、視覚的なテンポの変化も作っている
(`docs/04_PowerPoint.md`「動画としてのテンポ・視覚デザイン」参照)。事実・出典は
変更していない。
...
| `deck_spec.json` | 47枚のデッキ仕様(1スライド1メッセージ、同一レイアウト連続なし、bullets比率21.3%) |
| `deck.pptx` | 生成済みの.pptx本体。チャットで直接送付する |
| `narration_script.md` | 単独ナレーション原稿(47スライド、目安約2分) |
| `dialogue_spec.json` | AI対話ナレーション・スライド連動モード仕様(47スライド全カバー、54発言) |
| `dialogue_script.md` | 対話ナレーション原稿(47スライド、目安約3分) |
...
- `verify_pptx.py --min-slides 47 --max-slides 47` → 合格
- `build_deck.py`の同一レイアウト4連続以上チェック・bullets比率40%以下チェック(21.3%) → 合格
- `generate_dialogue_script.py --deck`のスライド数整合性検証(47枚、1〜47連番) → 合格
