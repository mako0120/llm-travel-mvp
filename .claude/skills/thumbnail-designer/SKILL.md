...
description: >
  【2026-07-24、オーナー指示により停止中】AI Company OSのテーマ制作で
  サムネイルを自動生成する自動トリガーは無効。新しいテーマを作った際に
  指示なくこのスキルを使わないこと。オーナーが明示的に「サムネイルを作って」
  「thumbnail-designerスキルを使って」と直接依頼した場合のみ、このファイル
  本文の手順(Canva経由/Pillowフォールバック)を参照して対応する。
---

# サムネイル・デザイナー(AI Company OS)〔停止中〕

**2026-07-24、オーナーより「サムネイルはあなた作らなくていいです」と指示が
あり、定期実行の自動パイプラインからサムネイル生成を外した。** 新しいテーマを
制作する際、指示がなくてもサムネイルまで自動生成していた挙動(旧仕様)は
廃止する。以後、定期実行の完了条件からもサムネイル関連の項目を外すこと。

オーナーから明示的にサムネイル作成を依頼された場合のみ、以下の手順(旧仕様、
参考として残す)を使う。
