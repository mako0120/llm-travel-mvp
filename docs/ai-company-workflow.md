...
`.github/workflows/ai-handoff-router.yml` がGitHubイベントを即時に受け取ります。

- Issueに `claude` が付くと `ai:claude-ready` を付け、Claude/Cowork向けの受付コメントを作成します。
- `claude/feature-*` のDraft PRを受け取ると `ai:codex-waiting` を付けます。
- PRがReadyになると `ai:codex-review` へ切り替えます。
- PR更新時も同じコメントを更新し、重複コメントを作りません。

このルーターはGitHub内の状態を即時更新します。Claude CodeやCodexの外部タスクを実際に起動するには、Coworkまたは各GitHub連携が上記ラベルを受信する設定が必要です。APIキーやWebhook Secretはリポジトリへ保存しません。

Claude Code側は `.github/workflows/claude-implement.yml` を有効にすると、`claude` ラベルから公式Claude Code GitHub Actionを直接起動できます。有効化方法と緊急停止は `docs/claude-automation-setup.md` に記載します。認証Secretと明示的なRepository variableが揃わない限り、Claude Codeは起動しません。
